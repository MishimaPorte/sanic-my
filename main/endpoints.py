...
async def admin_users(request: Request) -> HTTPResponse:
    u = await ApiUser.all()
    a = AUSerializer()
    a.accounts_serializer = AccSerializer(
        fields=("id", "balance", "transaction_history")
    )
    await a.s(u)
    return json(a.data)
...
async def admin_accounts_disable(request: Request) -> HTTPResponse:
    if request.json.get("user_ids", None):
        ...
        return json(
            {"error": "proide user_ids list of user ids to disable"}, status=400
        )
