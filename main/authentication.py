...
with open("../keys/id_rsa", "r") as first_keyfile:
    private_key = serialization.load_ssh_private_key(first_keyfile.read().encode(), password = b"")
...
async def signup(request, login: str = None, password: str = None):
    ...
    try:
        user, created = await ApiUser.get_or_create(
                login = login,
                password = hash(password, user_salt),
                salt = user_salt
        )
...
async def login(login: str, password: str, request):
    ...
    if not user == None:
        ...
        return (404, {"error": "Trying to log as a user that does not, technically, exist."})
