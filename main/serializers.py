        ...
        for item in qs:
            ...
            for field in fields:
                ...
                if issubclass(type(val), QuerySet):
                    val = await val
                    if hasattr(self, f"{field}_serializer"):#self.__hasattr__(f"{field}_serializer"):
                        await self.__getattribute__(f"{field}_serializer").s(val, many = False)
                        d.update({field: self.__getattribute__(f"{field}_serializer").data})
                    else:
